...
export class ExcelService {
  ...
  async generateExcel2() {
    ...
    this.http.get<any>('http://localhost:8000/api/notehis?filter[where][owner]=miguta').subscribe(resp => {
      // Add Data and Conditional Formatting
      let data = [];
      let datax = [];
      for (let x=0; x < resp.length; x++) {
        data.push(resp[x].id, resp[x].accnumber,resp[0].custnumber, resp[x].notemade,resp[x].owner,resp[x].noteimp,resp[x].notesrc,resp[x].notedate)
        datax.push(data)
        data = [];
      }
      ...
      workbook.xlsx.writeBuffer().then((data: any) => {
        const blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        fs.saveAs(blob, 'CustomerNotes.xlsx');
      });
    }, error => {
      //
    })
